...
If you're using Vue 2.x, please see [the Vue recipe](../recipes/vue.md) instead, or use the [`xstate-vue2` package](https://github.com/ChrisShank/xstate-vue2) if you want to use the Vue Composition API.
:::
    ...
    const { state, send } = useMachine(fetchMachine, {
      ...
      services: {
        fetchData: (_context, event) =>
          fetch(`some/api/${event.query}`).then((res) =>
            res.json()
          ),
      },
    });
...
<template>
  <div>
    ...
    <loader-loading-user
      v-if-else="state.matches({ loading: 'user' })"
    />
