...
## Метод автомата `.transition()` {#machine-transition-method}
...
## Переходы без смены состояния {#self-transitions}
...
## Проходные переходы {#transient-transitions}
...
## Безсобытийные "Always" переходы {#eventless-always-transitions}
...
## События по-умолчанию {#wildcard-descriptors}
