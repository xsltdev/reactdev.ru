...
```js
function FontButton() {
  const [fontSize, setFontSize] = useRecoilState(
    fontSizeState
  );
...
_Обратите внимание_, что мы используем обновляющую форму сеттера, поэтому имеем возможность создать новый список на основе старого.
...
_Обратите внимание_, что в UI отображаются все задачи, поскольку дефолтным значением `todoListFilterState` является "Show All". Для того, чтобы иметь возможность изменять фильтр, необходимо реализовать компонент `TodoListFilters`:
...
function CurrentUserInfo() {
  ...
  const setCurrentUserID = useSetRecoilState(
    currentUserIDState
  );
  return (
    <div>
...
```js
function UserInfo({ userID }) {
  const userNameLoadable = useRecoilValueLoadable(
...
const historyEffect = (name) => ({ setSelf, onSet }) => {
  onSet((newValue, oldValue) => {
    history.push({
      label: `${name}: ${JSON.serialize(
        oldValue
      )} -> ${JSON.serialize(newValue)}`,
      undo: () => {
        setSelf(oldValue);
      },
    });
...
```js
const syncStorageEffect = (userID) => ({
  setSelf,
  trigger,
}) => {
  // Инициализируем атом значением из удаленного хранилища состояния
  if (trigger === 'get') {
    // Предотвращаем дорогую инициализацию
...
_Обратите внимание_, что приводимый ниже пример намеренно упрощен и не охватывает всех возможных случаев:
...
`RecoilRoot` представляет независимого провайдера/хранилище состояния атома. _Обратите внимание_, что кэши, например, кэши селекторов могут распределяться между несколькими `root`.
...
function TempCelsius() {
  ...
  return (
    <div>
      ...
      <br />
      <button onClick={addTenCelsius}>
        Добавить 10 градусов Цельсия
      </button>
      <br />
      <button onClick={addTenFahrenheit}>
...
`Loadable` также содержит вспомогательные функции для получения текущего состояния. _Обратите внимание_, данный API является нестабильным:
...
function FormContent({ setNamesState }) {
  ...
  return (
    <>
      ...
      <button
        onClick={() =>
          setNamesState((names) => [...names, name])
        }
      >
        Добавить имя
      </button>
...
const TodoResetButton = () => {
  ...
  return (
    <button onClick={resetList}>Выполнить сброс</button>
  );
};
```
...
function UserInfo({ userID }) {
  const [
    userNameLoadable,
    setUserName,
  ] = useRecoilStateLoadable(userNameQuery(userID));
...
function CartInfoDebug() {
  const logCartItems = useRecoilCallback(
    ({ snapshot }) => async () => {
      const numItemsInCart = await snapshot.getPromise(
        itemsInCart
      );
      console.log(
...
function MyComponent() {
  ...
  const multipliedNumber = useRecoilValue(
    myMultipliedState(100)
  );
...
Семьи селекторов также полезны для передачи параметров в запросы. _Обратите внимание_, что селектор, используемый для абстрагирования запросов, должен быть "чистой" функцией:

```js
const myDataQuery = selectorFamily({
  key: 'MyDataQuery',
  get: (queryParameters) => async ({ get }) => {
    const response = await asyncDataRequest(
      queryParameters
    );
    if (response.error) {
      throw response.error;
...
function MyChart({ layerQueries }) {
  const layerLoadables = useRecoilValue(
    waitForNone(layerQueries)
  );
...
```js
function MyComponent() {
  const logState = useRecoilCallback(
    ({ snapshot }) => () => {
      console.log(
        'Состояние: ',
        snapshot.getLoadable(myAtom).contents
      );
