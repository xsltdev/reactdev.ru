...
Если ваш `<select>` является неконтролируемым, вы можете передать пропс `defaultValue` вместо него:

-   `defaultValue`: Строка (или массив строк для `multiple={true}`). Определяет первоначально выбранный вариант.

Эти пропсы `<select>` актуальны как для неконтролируемых, так и для контролируемых полей выбора:
