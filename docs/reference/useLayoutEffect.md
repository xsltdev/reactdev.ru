...
**`useLayoutEffect`** - это версия [`useEffect`](useEffect.md), которая срабатывает перед тем, как браузер перерисовывает экран.
...
### Я получаю ошибку: "`useLayoutEffect` ничего не делает на сервере"
...
Когда вы или ваш фреймворк использует [серверный рендеринг](server.md), ваше приложение React рендерит в HTML на сервере для первоначального рендеринга. Это позволяет вам показать начальный HTML до загрузки кода JavaScript.
