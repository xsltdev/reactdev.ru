...
!!!warning ""
    ...
    Использование `cloneElement` является необычным и может привести к хрупкому коду.

`cloneElement` позволяет вам создать новый элемент React, используя другой элемент в качестве отправной точки.
...
Вызывает `cloneElement` для создания элемента React, основанного на `element`, но с различными `props` и `children`:
...
-   `props`: Аргумент `props` должен быть либо объектом, либо `null`. Если вы передадите `null`, клонированный элемент сохранит все исходные `element.props`. В противном случае для каждого реквизита в объекте `props` возвращаемый элемент "предпочтет" значение из `props` значению из `element.props`. Остальные реквизиты будут заполнены из исходного `element.props`. Если вы передадите `props.key` или `props.ref`, они заменят исходные.
...
-   `cloneElement` затрудняет отслеживание потока данных, поэтому **попробуйте вместо него альтернативы.**.

## Использование

### Переопределение реквизитов элемента

Чтобы переопределить реквизиты некоторого реактивного элемента, передайте его в `cloneElement` с реквизитами, которые вы хотите переопределить:
...
Короче говоря, `List` клонирует полученные элементы `<Row />` и добавляет к ним дополнительный реквизит.

!!!warning ""

    При клонировании детей трудно определить, как данные проходят через ваше приложение. Попробуйте один из альтернативных вариантов.

## Альтернативы

### Передача данных с реквизитом рендеринга

Вместо того чтобы использовать `cloneElement`, подумайте о том, чтобы принимать _render prop_, например `renderItem`. Здесь `List` принимает `renderItem` в качестве реквизита. `List` вызывает `renderItem` для каждого элемента и передает `isHighlighted` в качестве аргумента:
...
Реквизит `renderItem` называют "реквизитом рендеринга", потому что он определяет, как рендерить что-либо. Например, вы можете передать реализацию `renderItem`, которая рендерит `<Row>` с заданным значением `isHighlighted`:
...
При таком подходе `Row` вообще не нужно получать реквизит `isHighlighted`. Вместо этого он считывает контекст:
