...
Это происходит потому, что [states ведет себя как моментальный снимок](../learn/state-as-a-snapshot.md). Обновление состояния запрашивает другой рендер с новым значением состояния, но не влияет на переменную JavaScript `count` в уже запущенном обработчике событий.
