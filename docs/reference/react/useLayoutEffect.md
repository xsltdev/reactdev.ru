...
-   В качестве альтернативы, [пометьте свой компонент как предназначенный только для клиентов](Suspense.md#providing-a-fallback-for-server-errors-and-client-only-content) Это говорит React заменить содержимое до ближайшей границы [`<Suspense>`](Suspense.md) на фаллбэк загрузки (например, спиннер или мерцание) во время серверного рендеринга.
