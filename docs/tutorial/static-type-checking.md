...
> Примечание:
>
> Дальнейшие инструкции рассчитаны на тех, кто _не использует_ Create React App, т.к. там уже есть все необходимые настройки для работы с Flow.
...
Кроме того, есть [возможность](https://flow.org/en/docs/config/options/#toc-all-boolean) заставить Flow проверять вообще _все_ файлы. Если вы переводите на Flow проект, в котором уже есть наработки кода, может возникнуть множество конфликтов, а вот для старта с нуля такая опция может стать неплохим выбором.
...
**[DefinitelyTyped](https://github.com/DefinitelyTyped/DefinitelyTyped)** — это внушительный репозиторий файлов объявлений. Например, React устанавливается без собственного файла объявления — вместо этого мы устанавливаем его отдельно:
