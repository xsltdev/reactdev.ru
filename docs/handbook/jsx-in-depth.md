...
Вы можете узнать больше про [условный рендеринг](conditional-rendering.md) и [циклы](lists-and-keys.md) в соответствующих разделах.
...
В приведённом выше примере, проп `kind` используется безопасно и _не_ передаётся в элемент `<button>`, находящийся в DOM.
