...
После создания компонентов у вас будет библиотека многократно используемых компонентов, которые отображают вашу модель данных. Поскольку это статическое приложение, компоненты будут возвращать только JSX. Компонент на вершине иерархии (`FilterableProductTable`) будет принимать вашу модель данных в качестве реквизита. Это называется _односторонним потоком данных_, потому что данные стекаются от компонента верхнего уровня к компонентам в нижней части дерева.
...
Какие из них являются государственными? Определите те, которые не являются:

-   Остается ли он **неизменным** с течением времени? Если да, то это не состояние.
-   Передается ли он **от родителя** через реквизиты? Если да, то это не состояние.
-   Можете ли вы вычислить его на основе существующего состояния или реквизита в вашем компоненте? Если да, то это определенно не состояние!

То, что осталось - это, вероятно, состояние.

Давайте пройдемся по ним по одному:

1.  Исходный список продуктов _передается как реквизит, поэтому это не состояние_.
...
3.  Решите, где должно находиться состояние:
    ...
    2.  Вы также можете поместить государство в какой-либо компонент над их общим родителем.
...
Затем передайте `filterText` и `inStockOnly` в `ProductTable` и `SearchBar` в качестве реквизитов:
...
Вы предоставили `value` свойство полю формы без `onChange` обработчика. Это приведет к тому, что поле будет доступно только для чтения.

В приведенной выше песочнице, `ProductTable` и `SearchBar` читают реквизиты `filterText` и `inStockOnly` для рендеринга таблицы, input и checkbox. Например, вот как `SearchBar` заполняет значение ввода:
...
В настоящее время ваше приложение отображается правильно с реквизитами и состоянием, перетекающими вниз по иерархии. Но чтобы изменить состояние в соответствии с пользовательским вводом, вам нужно поддерживать поток данных в обратном направлении: компоненты формы в глубине иерархии должны обновить состояние в `FilterableProductTable`.
