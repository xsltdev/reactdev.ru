...
Не имеет значения, откуда берутся `page` и `query`. Пока этот компонент виден, вы хотите сохранить `результаты` [синхронизированными](synchronizing-with-effects.md) с данными из сети для текущей `страницы` и `запроса`. Вот почему это Эффект.
...
### 1. Преобразование данных без эффектов
...
=== "Результат"
    ...
    ![Результат](you-might-not-need-an-effect-1.png)
...
### 2. Кэширование расчета без эффектов
...
### 3. Сброс состояния без эффектов
...
### 4. Отправить форму без эффектов
