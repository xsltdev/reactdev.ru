...
Эффекты _действительно_ нужны для [синхронизации](synchronizing-with-effects.md) с внешними системами. Например, вы можете написать эффект, который синхронизирует виджет jQuery с состоянием React. С помощью эффектов можно также получать данные: например, синхронизировать результаты поиска с текущим поисковым запросом. Имейте в виду, что современные [фреймворки](start-a-new-react-project.md) предоставляют более эффективные встроенные механизмы получения данных, чем написание Эффектов непосредственно в ваших компонентах.

Чтобы помочь вам обрести правильную интуицию, давайте рассмотрим несколько распространенных конкретных примеров!

### Обновление состояния на основе пропсов или состояния {#updating-state-based-on-props-or-state}

Предположим, у вас есть компонент с двумя переменными состояния: `firstName` и `lastName`. Вы хотите вычислить из них `fullName` путем их конкатенации. Более того, вы хотите, чтобы `fullName` обновлялось при каждом изменении `firstName` или `lastName`. Первое, что приходит в голову, - добавить переменную состояния `fullName` и обновлять ее в Effect:
...
**Если что-то можно вычислить из существующих пропсов или состояния, [не помещайте это в состояние](choosing-the-state-structure.md) Вместо этого вычисляйте это во время рендеринга.** Это сделает ваш код быстрее (вы избежите лишних "каскадных" обновлений), проще (вы удалите часть кода) и менее подверженным ошибкам (вы избежите ошибок, вызванных рассинхронизацией различных переменных состояния друг с другом). Если такой подход кажется вам новым, в [Мыслим как React](thinking-in-react.md) объясняется, что должно входить в состояние.

### Кэширование дорогих вычислений {#caching-expensive-calculations}

Этот компонент вычисляет `visibleTodos`, принимая `todos`, которые он получает по пропсам, и фильтруя их в соответствии с пропсом `filter`. У вас может возникнуть желание хранить результат в состоянии и обновлять его из Effect:
...
Обычно этот код является нормальным! Но, возможно, `getFilteredTodos()` работает медленно или у вас много `todos`. В таком случае вы не хотите пересчитывать `getFilteredTodos()`, если изменилась какая-то несвязанная переменная состояния, например `newTodo`.

Вы можете кэшировать (или ["memoize"](https://ru.wikipedia.org/wiki/%D0%9C%D0%B5%D0%BC%D0%BE%D0%B8%D0%B7%D0%B0%D1%86%D0%B8%D1%8F)) дорогостоящее вычисление, обернув его в хук [`useMemo`](../reference/useMemo.md):
...
Функция, которую вы обернули в [`useMemo`](../reference/useMemo.md), запускается во время рендеринга, поэтому это работает только для [чистых вычислений](keeping-components-pure.md).
...
!!!note "Как определить, является ли вычисление дорогим?"
    ...
    Также обратите внимание, что измерение производительности в процессе разработки не даст вам наиболее точных результатов. (Например, если включен [Строгий режим](../reference/StrictMode.md), каждый компонент будет отображаться дважды, а не один раз). Чтобы получить наиболее точные результаты, создайте приложение для производства и протестируйте его на устройстве, которое есть у ваших пользователей.

### Сброс всех состояний при изменении пропса {#resetting-all-state-when-a-prop-changes}

Этот компонент `ProfilePage` получает пропс `userId`. Страница содержит входной комментарий, и вы используете переменную состояния `comment` для хранения его значения. Однажды вы заметили проблему: когда вы переходите от одного профиля к другому, состояние `comment` не сбрасывается. В результате легко случайно опубликовать комментарий в профиле не того пользователя. Чтобы решить эту проблему, вы хотите очищать переменную состояния `comment` при каждом изменении `userId`:
...
POST-запрос аналитики должен оставаться в Эффекте. Это потому, что _причина_ отправки события аналитики заключается в том, что форма была отображена. При разработке оно сработает дважды, но [см. здесь](synchronizing-with-effects.md) о том, как с этим справиться.
...
Это намного эффективнее. Кроме того, если вы реализуете способ просмотра истории игры, теперь вы сможете установить каждую переменную состояния на ход из прошлого, не запуская цепочку Effect, которая корректирует все остальные значения. Если вам нужно повторно использовать логику между несколькими обработчиками событий, вы можете извлечь функцию и вызвать ее из этих обработчиков.

Помните, что внутри обработчиков событий [состояние ведет себя как снимок](state-as-a-snapshot.md). Например, даже после вызова `setRound(round + 1)`, переменная `round` будет отражать значение на момент нажатия пользователем кнопки. Если вам нужно использовать следующее значение для вычислений, определите его вручную, например, `const nextRound = round + 1`.
...
При таком подходе и компонент `Toggle`, и его родительский компонент обновляют свое состояние во время события. React [batches updates](queueing-a-series-of-state-updates.md) от разных компонентов вместе, так что будет только один проход рендеринга.
...
["Lifting state up"](sharing-state-between-components.md) позволяет родительскому компоненту полностью контролировать `Toggle`, переключая собственное состояние родительского компонента. Это означает, что родительский компонент будет содержать больше логики, но в целом будет меньше состояния, о котором нужно беспокоиться. Всякий раз, когда вы пытаетесь синхронизировать две разные переменные состояния, попробуйте поднять состояние вверх!
...
Хотя обычно для этого используются эффекты, в React есть специально созданный Hook для подписки на внешнее хранилище, который предпочтительнее использовать. Удалите эффект и замените его вызовом [`useSyncExternalStore`](../reference/useSyncExternalStore.md):
...
Этот подход менее подвержен ошибкам, чем ручная синхронизация изменяемых данных с состоянием React с помощью Effect. Обычно вы пишете пользовательский хук, подобный `useOnlineStatus()` выше, чтобы вам не нужно было повторять этот код в отдельных компонентах. [Подробнее о подписке на внешние хранилища из компонентов React.](../reference/useSyncExternalStore.md)
...
Не имеет значения, откуда берутся `page` и `query`. Пока этот компонент виден, вы хотите сохранить `results` [синхронизированными](synchronizing-with-effects.md) с данными из сети для текущей `page` и `query`. Вот почему это Эффект.

Однако в приведенном выше коде есть ошибка. Представьте, что вы быстро набираете `"hello"`. Тогда `запрос` изменится с `"h"` на `"he"`, `"hel"`, `"hell"` и `"hello"`. Это приведет к запуску отдельных запросов, но нет никакой гарантии, в каком порядке будут получены ответы. Например, ответ `"hell"` может прийти _после_ ответа `"hello"`. Поскольку вызов `setResults()` будет последним, вы отобразите неправильные результаты поиска. Это называется ["условие гонки"](https://ru.wikipedia.org/wiki/%D0%A1%D0%BE%D1%81%D1%82%D0%BE%D1%8F%D0%BD%D0%B8%D0%B5_%D0%B3%D0%BE%D0%BD%D0%BA%D0%B8): два разных запроса "мчатся" друг за другом и приходят в другом порядке, чем вы ожидали.

**Чтобы исправить состояние гонки, вам нужно [добавить функцию очистки](synchronizing-with-effects.md) для игнорирования несвежих ответов:**.
...
**Эти проблемы относятся к любой библиотеке пользовательского интерфейса, не только к React. Их решение не является тривиальным, поэтому современные [фреймворки](start-a-new-react-project.md) предоставляют более эффективные встроенные механизмы выборки данных, чем выборка данных в Effects.**.
...
!!!note "Итого"
    ...
    -   Чтобы сбросить определенный бит состояния в ответ на изменение пропса, установите его во время рендеринга.
...
### 1. Преобразование данных без эффектов {#transform-data-without-effects}
...
=== "CodeSandbox"
    ...
    <iframe src="https://codesandbox.io/embed/5wzflz?view=Editor+%2B+Preview&module=%2Fsrc%2FApp.js" style="width:100%; height: 500px; border:0; border-radius: 4px; overflow:hidden;" title="react.dev" allow="accelerometer; ambient-light-sensor; camera; encrypted-media; geolocation; gyroscope; hid; microphone; midi; payment; usb; vr; xr-spatial-tracking" sandbox="allow-forms allow-modals allow-popups allow-presentation allow-same-origin allow-scripts"></iframe>
...
???success "Показать решение"
    ...
    === "CodeSandbox"
    	...
    	<iframe src="https://codesandbox.io/embed/376plc?view=Editor+%2B+Preview&module=%2Fsrc%2FApp.js" style="width:100%; height: 500px; border:0; border-radius: 4px; overflow:hidden;" title="react.dev" allow="accelerometer; ambient-light-sensor; camera; encrypted-media; geolocation; gyroscope; hid; microphone; midi; payment; usb; vr; xr-spatial-tracking" sandbox="allow-forms allow-modals allow-popups allow-presentation allow-same-origin allow-scripts"></iframe>

### 2. Кэширование расчета без эффектов {#cache-a-calculation-without-effects}
...
???success "Показать решение"
    ...
    === "CodeSandbox"
    	...
    	<iframe src="https://codesandbox.io/embed/44jsfw?view=Editor+%2B+Preview&module=%2Fsrc%2FApp.js" style="width:100%; height: 500px; border:0; border-radius: 4px; overflow:hidden;" title="react.dev" allow="accelerometer; ambient-light-sensor; camera; encrypted-media; geolocation; gyroscope; hid; microphone; midi; payment; usb; vr; xr-spatial-tracking" sandbox="allow-forms allow-modals allow-popups allow-presentation allow-same-origin allow-scripts"></iframe>
    ...
    === "CodeSandbox"
    	...
    	<iframe src="https://codesandbox.io/embed/y48vn9?view=Editor+%2B+Preview&module=%2Fsrc%2FApp.js" style="width:100%; height: 500px; border:0; border-radius: 4px; overflow:hidden;" title="react.dev" allow="accelerometer; ambient-light-sensor; camera; encrypted-media; geolocation; gyroscope; hid; microphone; midi; payment; usb; vr; xr-spatial-tracking" sandbox="allow-forms allow-modals allow-popups allow-presentation allow-same-origin allow-scripts"></iframe>
...
### 3. Сброс состояния без эффектов {#reset-state-without-effects}
...
???success "Показать решение"
    ...
    === "CodeSandbox"
    	...
    	<iframe src="https://codesandbox.io/embed/hk6y7g?view=Editor+%2B+Preview&module=%2Fsrc%2FEditContact.js" style="width:100%; height: 500px; border:0; border-radius: 4px; overflow:hidden;" title="react.dev" allow="accelerometer; ambient-light-sensor; camera; encrypted-media; geolocation; gyroscope; hid; microphone; midi; payment; usb; vr; xr-spatial-tracking" sandbox="allow-forms allow-modals allow-popups allow-presentation allow-same-origin allow-scripts"></iframe>

### 4. Отправить форму без эффектов {#submit-a-form-without-effects}
...
=== "CodeSandbox"
    ...
    <iframe src="https://codesandbox.io/embed/j7dtd8?view=Editor+%2B+Preview&module=%2Fsrc%2FApp.js" style="width:100%; height: 500px; border:0; border-radius: 4px; overflow:hidden;" title="react.dev" allow="accelerometer; ambient-light-sensor; camera; encrypted-media; geolocation; gyroscope; hid; microphone; midi; payment; usb; vr; xr-spatial-tracking" sandbox="allow-forms allow-modals allow-popups allow-presentation allow-same-origin allow-scripts"></iframe>
...
???success "Показать решение"
    ...
    === "CodeSandbox"
    	...
    	<iframe src="https://codesandbox.io/embed/s7ggjl?view=Editor+%2B+Preview&module=%2Fsrc%2FApp.js" style="width:100%; height: 500px; border:0; border-radius: 4px; overflow:hidden;" title="react.dev" allow="accelerometer; ambient-light-sensor; camera; encrypted-media; geolocation; gyroscope; hid; microphone; midi; payment; usb; vr; xr-spatial-tracking" sandbox="allow-forms allow-modals allow-popups allow-presentation allow-same-origin allow-scripts"></iframe>
...
<small>:material-information-outline: Источник &mdash; [https://react.dev/learn/you-might-not-need-an-effect](https://react.dev/learn/you-might-not-need-an-effect)</small>
