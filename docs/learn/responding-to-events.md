...
### Фаза захвата {#capture-phase-events}

Обработчики с суффиксом `Capture` в имени (например, `onClickCapture`) вызываются на фазе захвата, до того как событие достигнет цели и всплывёт. Обычные обработчики без `Capture` участвуют во всплытии (фаза всплытия).
