...
<small>Перевод документации [React v16](../tutorial/tutorial.md)</small>
...
Информация, которую вы передаете таким образом, называется _реквизитами_. Теперь компонент `MyApp` содержит состояние `count` и обработчик события `handleClick`, и _передает их в качестве реквизитов_ каждой из кнопок.

Наконец, измените `MyButton`, чтобы он _читал_ реквизиты, которые вы передали из его родительского компонента:
