...
Когда вы хотите, чтобы компонент "запомнил" какую-то информацию, но не хотите, чтобы эта информация [запускала новые рендеры] (/learn/render-and-commit), вы можете использовать _ref_.
...
Ссылка указывает на число, но, как и [state](/learn/state-a-components-memory), вы можете указать на что угодно: строку, объект или даже функцию. В отличие от state, ref - это обычный объект JavaScript со свойством `current`, который можно читать и изменять.
