...
-   Как добавить переменную состояния с помощью хука [`useState`](../reference/react/useState.md)
...
Хук [`useState`](../reference/react/useState.md) обеспечивает эти две вещи:
...
> Синтаксис `[` и `]` здесь называется [деструктуризация массива](https://javascript.info/destructuring-assignment) и позволяет вам читать значения из массива. Массив, возвращаемый `useState`, всегда содержит ровно два элемента.
...
**Крючки-функции, начинающиеся с `use`, можно вызывать только на верхнем уровне ваших компонентов или [ваших собственных крючков](reusing-logic-with-custom-hooks.md)** Вы не можете вызывать крючки внутри условий, циклов или других вложенных функций. Hooks - это функции, но полезно думать о них как о безусловных декларациях о потребностях вашего компонента. Вы "используете" функции React в верхней части вашего компонента подобно тому, как вы "импортируете" модули в верхней части вашего файла.

### Анатомия `useState`

Когда вы вызываете [`useState`](../reference/react/useState.md), вы говорите React, что хотите, чтобы этот компонент что-то запомнил:
...
Обратите внимание, что `hasPrev` и `hasNext` используются _как_ для возвращаемого JSX, так и внутри обработчиков событий! Эта удобная схема работает потому, что функции обработчиков событий ["закрывают"](https://developer.mozilla.org/ru/docs/Web/JavaScript/Closures) любые переменные, объявленные во время рендеринга.

#### Исправление застрявших вводов формы
...
Существуют ли какие-либо ограничения на _где_ могут быть вызваны крючки? Нарушает ли этот компонент какие-либо правила? Проверьте, нет ли комментариев, отключающих проверку линтера - именно здесь часто скрываются ошибки!
...
Вы также можете удалить ненужную ветвь `else`, чтобы уменьшить вложенность. Однако по-прежнему важно, чтобы все вызовы крючков происходили _до_ первого `возврата`.
